...
async function sendPromptToSession({ req, res, context, requestId, text, session }) {
  ...
  try {
    ...
    await refreshSessionOutputWithBudget({
      provider,
      state,
      config,
...
async function refreshSessionOutputWithBudget({
  ...
  if (!budgetMs) {
    return { result: await pending, budgetExceeded: false };
  }
  let timer;
  ...
  if (raced !== OUTPUT_BUDGET_EXCEEDED) {
    return { result: raced, budgetExceeded: false };
  }
  pending.catch(() => {});
  ...
  return { result: null, budgetExceeded: true };
}
